...
export default function setPasswordPage() {
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    setIsLoading(true);
    try {
      const staffAccount = await createStaffAccount(
        accountId,
        user?.name,
        email,
        values.password
      ...
      // Call your API route to set the password
      ...
      if (res.ok && result.success) {
        try {
          const verifyResult = await verifyAccount(userId);
          if (verifyResult) {
            setTimeout(() => {
              toast.success("Password set successful! Redirecting to Dashboard.");
              router.push("/admin");
            }, 2000);
          }
        } catch (err) {
