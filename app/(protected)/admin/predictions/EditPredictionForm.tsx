...
const EditPredictionForm = ({ prediction }: { prediction: any }) => {
  ...
  const form = useForm<z.infer<typeof formSchema>>({
    ...
    defaultValues: {
      ...
      tip: prediction.tip,
      subscriptionType: prediction.subscriptionType,
      odd: prediction.odd,
      over: prediction.over === null ? "" : prediction.over,
      chance: prediction.chance === null ? "" : prediction.chance,
      htft: prediction.htft === null ? "" : prediction.htft,
      either: prediction.either === null ? "" : prediction.either,
      isBanker: prediction.isBanker,
      isBtts: prediction.isBtts,
    },
  });

  const [sportTypeValue, setSportTypeValue] = useState(prediction.sportType);

  const { mutateAsync: editPrediction, isPending: isLoading } =
    useEditPrediction();
              ...
              <FormItem>
                <FormControl>
                  <Input
                    ...
                    className="w-full text-sm placeholder:text-sm bg-stone-100 rounded-sm border-secondary overflow-auto"
                    {...field}
                  />
                </FormControl>
                <FormMessage />
